// CryptoJS Library Bundle for AES Decryption
...
class DefaultExtension extends MProvider {
    ...
    async getPopular(page) {
        ...
        for (const item of items) {
            const a = item.querySelector("h2 a, h3 a, .left a, .thm-effect");
    ...
    async getLatestUpdates(page) {
        const url = `https://www.mangago.me/genre/all/${page}/?f=1&o=1&sortby=update_date&e=`;
        const res = await this.client.get(url, this.getHeaders());
        const doc = new Document(res.body);
        const list = [];
        const items = doc.querySelectorAll(".updatesli, .flex1.listitem, #information li, .pic_list li");
    ...
    async search(query, page, filters) {
        ...
        if (query && query.trim().length > 0) {
            url = `https://www.mangago.me/r/l_search/?name=${encodeURIComponent(query)}&page=${page}`;
        } else {
            let genre = "all";
            let sortby = "view";
            let status = "";
            if (filters && Array.isArray(filters)) {
                for (const filter of filters) {
                    if (filter.type_name === "SelectFilter" && filter.name === "Sort By") {
                        sortby = filter.values[filter.state]?.value ?? "view";
                    } else if (filter.type_name === "SelectFilter" && filter.name === "Status") {
                        status = filter.values[filter.state]?.value ?? "";
                    } else if (filter.type_name === "SelectFilter" && filter.name === "Genre") {
                        genre = filter.values[filter.state]?.value ?? "all";
                    }
                }
            }
            url = `https://www.mangago.me/genre/${genre}/${page}/?f=1&o=1&sortby=${sortby}&status=${status}&e=`;
        }
    ...
    async getDetail(url) {
        ...
        const titleElem = doc.querySelector(".w-title h1, #information h1, .manga_title h1");
        const title = titleElem ? titleElem.text.trim() : "";

        const imgElem = doc.querySelector("#information img, .cover img, .manga_pic img");
        var imageUrl = imgElem ? (imgElem.attr("data-src") || imgElem.attr("src") || imgElem.attr("data-original") || "") : "";
        if (imageUrl.startsWith("//")) imageUrl = `https:${imageUrl}`;

        const descElem = doc.querySelector(".manga_summary, #information .manga_summary, .summary");
        const description = descElem ? descElem.text.trim() : "";

        const authorElem = doc.querySelector(".manga_info li:contains(Author) a, #information li:contains(Author) a");
        const author = authorElem ? authorElem.text.trim() : "";

        const genreElems = doc.querySelectorAll(".manga_info li:contains(Genre) a, #information li:contains(Genre) a");
        const genres = [];
    ...
    _deobfuscateSoJsonV4(jsCode) {
        try {
            if (jsCode.indexOf("['sojson.v4']") === -1) return jsCode;
            const start = jsCode.indexOf("['sojson.v4']");
            const trimmed = jsCode.substring(start);
            const body = trimmed.substring(240, trimmed.length - 59);
            const parts = body.split(/[a-zA-Z]+/);
            let out = "";
            for (let i = 0; i < parts.length; i++) {
                if (parts[i].length > 0 && !isNaN(parts[i])) {
                    out += String.fromCharCode(parseInt(parts[i], 10));
                }
            }
            return out;
        } catch (_) {
