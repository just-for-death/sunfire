...
import '../../../widgets/local_download_button.dart';
...
class ChapterListTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Show the local download button on every platform that has a real
    // filesystem (Android, iOS, desktop). Only skip on web (no local storage).
    final showLocalButton = !kIsWeb;

    final trailing = showLocalButton
        ? Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              // ── Local (device) download button ─────────────────────────
              LocalDownloadButton(chapterId: chapter.id),
              const SizedBox(width: 2),
              // ── Server download / queue button ──────────────────────────
              DownloadStatusIcon(
