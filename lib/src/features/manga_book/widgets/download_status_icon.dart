...
class DownloadStatusIcon extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (isMobile) {
      ...
      // Show spinner while a server-triggered reload is pending.
      if (isLoading.value) {
        return Padding(
      ...
      Future<void> serverDeleteFinished() async {
      ...
      // ── Server download icon / button ─────────────────────────────────
      // The LocalDownloadButton (in chapter_list_tile.dart) handles offline
      // actions, so this icon only needs to cover server-side states.
